...
function Swap(props) {
  ...
  const { data, sendTransaction } = useSendTransaction({
    request: {
      ...
      gasPrice: String(txDetails.gasPrice),
    },
  });
  ...
  async function fetchDexSwap() {
    const allowanceResponse = await axios.get(
      `http://localhost:3001/api/allowance`,
      {
        params: {
    ...
    // const allowance = await axios.get(urlAllowance, configAllowance);
    setTimeout(async () => {
      if (allowance === "0") {
        const approve = await axios.get(
          `http://localhost:3001/api/approveTransaction?tokenAddress=${tokenOne.address}`
        );
        console.log(approve.data);
        const approvedData = approve.data;
        console.log(approvedData, "approvedData");
        const parseData = JSON.parse(approvedData);
        setTxDetails({
          to: parseData.to,
          data: parseData.data,
          value: parseData.value,
          gasPrice: parseData.gasPrice,
        });
        console.log("2345678", txDetails);
        console.log("not approved");
        // sendTransaction( );
        return;
      }
    ...
    //Swapping while routing
    try {
      const tx = await axios.get(`http://localhost:3001/api/swap`, {
        params: {
    ...
    //   `https://api.1inch.io/v6.0/1/swap?fromTokenAddress=${
    //     tokenOne.address
  ...
  // console.log(tokenOneAmount,"sdfgthyujki");
  // console.log(txDetails, "sdvdffff");
  ...
  // console.log(txDetails, "txDetailstxDetails");
